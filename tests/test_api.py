...
async def test_engine_and_sessionmaker():
    db_fd, db_path = tempfile.mkstemp(prefix="recipes_test_", suffix=".db")
    os.close(db_fd)
...
async def app_with_overrides(test_engine_and_sessionmaker):
    _, SessionMaker = test_engine_and_sessionmaker
